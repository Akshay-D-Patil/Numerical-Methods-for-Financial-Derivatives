...
Purpose             : The objective of this Python program is to solve a tridiagonal 
                      system of equations using the different algorithms thereby 
                      pricing the european and american options. We discretize the 
                      heat equation. Once we discretize the heat equation,we create 
                      the mesh for space and time and then use that mesh as a tridiagonal 
                      system. The tridiagonal system can be solved using algorithms 
                      such as Thomas, Brennan-Schwartz, SOR and PSOR. The different 
                      type of FDMs is decided by the value of the theta. 
                         
Numerical Methods   : Solving Tridiagonal system of equations, 
                      Thomas Algorithm(Gaussian Elimination), Brennan-Schwartz 
                      Algorithm, SOR Algorithm, PSOR Algorithm. Finite Difference 
                      Methods such as Explicit, Implicit and Crank-Nicholson.
...
def expl(S0, K, r, q, sigma, T, xmin, xmax, dx, dtau, theta, OptionInd, OptionType):
    '''
    Parameters
    ==========
    S0 : float
    ...
    if theta == 0: # Explicit FDM
        ...
        if OptionInd == 1: # Indicator for call option
            ...
            for j in range(M): # time loop
               ...
               # Indicator for solving european call options(Thomas algorithm)           
                if OptionType == 1: 
                        
                    w_mesh[1:N,j+1] = thomas(alpha,gamma,beta,b,N-1)
                    
                    
              # Indicator for solving american call options(brennan algorithm)                        
                elif OptionType == 0:
        ...
        # Indicator for solving european put options(Thomas algorithm)                
                if OptionType == 1: 
                        
                    w_mesh[1:N,j+1] = thomas(alpha,gamma,beta,b,N-1)
                    
        # Indicator for solving american put options(brennan algorithm)                     
                elif OptionType == 0: 
