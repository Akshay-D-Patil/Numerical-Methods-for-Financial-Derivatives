...
from hw7_PatilAkshay_FFTfunc import *
...
#        Approximating the strike price K for the calculation of option prices
        call_price[i] = approx(K, call_k, strike_call)
        put_price[i] = approx(K, put_k, strike_put)
 
       
#     Plot relation of V to K in each iteration 
        
        graph_fig = plt.figure()
        plt.suptitle ("European option prices for varying alphas = {0}".format(alpha[i],\
                      fontsize = 12))
 ...
 # Plot call graph
        ...
        call_graph.set_title("Call option price after scaling = {0}".format(graph_scale_call),\
                             fontsize=10)
 ...
 # Plot put graph
        ...
        put_graph.set_title("Put option price after scaling = {0}".format(graph_scale_put),\
                            fontsize=10)
